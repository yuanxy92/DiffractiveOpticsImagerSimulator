...
nupy_filename = './data/240722_Ep30+200_Dataset46_BS4_init0_theta28_lr4_D200_try6_high_fre/final_params_best/duty.npy'
output_dir = './results/in_glass_20240723_f_1150um_lambdascan_high_fre'
aperture_diameter = 0.2e-3
params['fab_noise_level_random'] = 0.10
params['fab_noise_level_sys'] = 0.0
...
prop_dist = []
for idx in range(1):
    prop_dist.append((1.15 + idx*0.025) * 1e-3)
# psf cropping size
crop_size = 601
...
if 'fab_noise_level_random' in params or 'fab_noise_level_sys' in params:
    print('Fabrication noise simulation on!')
    duty = add_noise(duty, params['fab_noise_level_random'], params['fab_noise_level_sys'])
    duty = duty * aperture_mask_
    cv2.imwrite(f'{output_dir}/duty_w_noise.png', (duty * 255).astype(np.uint8))
...
lambda_shifts = [-15, -10, -5, 0, 5, 10, 15]
